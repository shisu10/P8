...
async def user_info(req: Request):
    ...
    """
    user_da = User_DA()
    user_data = await user_da.get_by_id(req.state.user_id)
    if not user_data:
...
async def user_add(post: CreateUser):
    ...
    create_user = await user_da.create(**post.dict())
    if not create_user:
...
async def user_del(user_id: int):
    ...
    """
    user_da = User_DA()
    deleted = await user_da.delete(user_id)
    if not deleted:
...
async def account_login(post: AccountLogin):
    ...
    """
    user_da = User_DA()
    get_user = await user_da.get_by_username(post.username)
    if not get_user:
